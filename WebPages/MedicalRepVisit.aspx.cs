...
public partial class WebPages_MedicalRepVisit : System.Web.UI.Page
{
    ...
    protected void rdbType_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (rdbType.SelectedIndex == 0)
        {
            ...
            //string radconfirmscript = "<script language='javascript'> var prm = Sys.WebForms.PageRequestManager.getInstance();  " + "prm.add_endRequest(function () { " +
            //                           " LoadAllFunctions(); " +
            //                            " });</script>";
           ...
           // Page.ClientScript.RegisterStartupScript(this.GetType(), "radconfirm", radconfirmscript);
            ...
            //sb.Append("LoadAllFunctions(); ");
            //sb.Append(@"</script>");
            //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Message", sb.ToString(), false);
        }
...
" inner join tbl_Transaction on Trans_Mid=MedTrans_TransMid" +
 " where MedTrans_AF = 1 and MedTrans_ItemMid='" + ItemMid + "' and MedTrans_RepMid='" + UserId + "'),0)) as availqty";
            string Qry = "  select * from dbo.tbl_ProductMaster where Prod_Name='" + txtMedicine.Text.Replace("'","''")+ "'";
